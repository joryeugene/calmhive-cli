...
class ConfigManager {
  ...
  getStatus() {
    const status = {
      ...
      cliTemplateExists: fs.existsSync(this.getTemplatePath(false)),
      desktopTemplateExists: fs.existsSync(this.getTemplatePath(true))
    };
    ...
    if (status.configExists) {
      ...
      try {
        ...
        // Detect if it's desktop optimized
        status.isDesktop = content.includes('Desktop Optimized');
      } catch (error) {
        status.version = 'unknown';
        status.isDesktop = false;
      }
    }
   ...
   */
  async getDiff(desktop = false) {
    const templatePath = this.getTemplatePath(desktop);
   ...
   */
  async installConfig(desktop = false, dryRun = false) {
    const templatePath = this.getTemplatePath(desktop);
    const templateName = desktop ? 'CLAUDE-DESKTOP.md.example' : 'CLAUDE.md.example';
   ...
   */
  validateTemplate(desktop = false) {
    const templatePath = this.getTemplatePath(desktop);
    ...
    try {
      ...
      return {
        ...
        size: content.length,
        isDesktop: content.includes('Desktop Optimized')
      };
    } catch (error) {
  ...
  /**
   * Auto-detect if desktop environment
   */
  detectEnvironment() {
    // Check if running in terminal or desktop
    const hasDisplay = process.env.DISPLAY || process.env.WAYLAND_DISPLAY;
    const isMac = process.platform === 'darwin';
    const isWindows = process.platform === 'win32';
    
    // Heuristics for desktop environment
    const likelyDesktop = hasDisplay || isMac || isWindows;
    
    return {
      platform: process.platform,
      hasDisplay: !!hasDisplay,
      likelyDesktop,
      recommendation: likelyDesktop ? 'desktop' : 'cli'
    };
  }
