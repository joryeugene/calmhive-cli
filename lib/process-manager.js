  ...
  async runSingleIteration(session, iterationNum, logStream, adaptiveRetry) {
    return new Promise((resolve) => {
      ...
      claudeProcess.stdout.on('data', (data) => {
        ...
        if (stdoutOutput.includes('Context low') || stdoutOutput.includes('/compact')) {
          console.log(`🔄 Context limit detected, will restart with fresh context for next iteration`);
...
lets bee friends`;
      ...
      claudeProcess.on('exit', (code) => {
        ...
        // Check if exit was due to usage limits (check both stdout and stderr)
        if (((stdoutOutput.includes('Claude Max usage limit reached') || 
              stdoutOutput.includes('Your limit will reset at') ||
              stderrOutput.includes('Claude Max usage limit reached') || 
              stderrOutput.includes('Your limit will reset at')) && code !== 0) ||
            (stderrOutput.includes('usage limit') && code !== 0)) {
          console.log(`⚠️ Usage limit hit - will retry with exponential backoff`);
          adaptiveRetry.recordFailure();
          // Return false to trigger retry logic with proper delay
          resolve(false);
        }
        // Check if exit was due to context issues (non-zero exit but not a hard failure)
        else if (code === 1 && iterationNum > 1 && !session.contextResetAttempted) {
