...
let processedMessages = new Set(); // Track messages we've already injected into
...
/**
 * Generate a simple hash for user message content
 */
function hashUserMessage(content) {
  if (!content) {return null;}
  // Simple hash for conversation turn detection
  return content.toString().length + '_' + content.toString().slice(0, 50).replace(/\s+/g, '');
}
...
function isTypingContinuation(currentContent) {
  ...
  for (const recent of recentTyping) {
    ...
    // Check if current message is an extension of recent message
    ...
    // Check if current message is a prefix of recent message (backspacing)
...
function getConversationSessionId(bodyData) {
  ...
  // For single message conversations, generate session ID from the message
  ...
  // Use the first message content as session identifier
  ...
  // Only track as session if the first message was already injected
  if (firstContent.includes('CLAUDE.md RULES:')) {
    // Extract original message from injected content
  ...
  return null; // Don't track sessions for non-injected first messages
}
...
function markConversationInjected(bodyData) {
  ...
  if (sessionId) {
    ...
    for (const [id, data] of conversationSessions.entries()) {
