...
function shouldInjectForRequest(bodyData) {
  // NEVER inject if request has tools (this is a tool call)
  if (bodyData.tools && bodyData.tools.length > 0) {
    if (debugMode) {
      console.error('[Calmhive Debug] Skipping injection - request has tools (tool call)');
    }
    return false;
  }
