...
function shouldInjectForRequest(bodyData) {
  const analysis = analyzeRequestType(bodyData);
  ...
  // Only inject for fresh user messages or short conversations without recent tool use
  const shouldInject = analysis.isUserMessage || 
    (analysis.requestType === 'continued-conversation' && !analysis.hasRecentToolUse);
  ...
  if (debugMode) {
    console.error(`[Calmhive Debug] Injection decision: ${shouldInject ? 'INJECT' : 'SKIP'} (${analysis.requestType})`);
  }
