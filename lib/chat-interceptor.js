 ...
 * - CALMHIVE_DEBUG or CALMHIVE_INTERCEPTOR_DEBUG: Enable debug logging
 * - CALMHIVE_INJECTION_MODE: Control injection behavior
 *   - 'always': Inject on every request (legacy behavior)
 *   - 'smart': Skip automated/tool contexts (default)
 *   - 'interactive-only': Only inject for fresh user messages
 *   - 'never': Disable injection completely
 * - CALMHIVE_MAX_INJECTIONS: Max injections per session (default: 1)
 * - CALMHIVE_INJECTION_COOLDOWN: Cooldown between injections in ms (default: 300000 = 5 min)
 */
...
// Environment variable controls for injection behavior
const injectionMode = process.env.CALMHIVE_INJECTION_MODE || 'smart'; // 'always', 'smart', 'interactive-only', 'never'
const maxInjectionsPerSession = parseInt(process.env.CALMHIVE_MAX_INJECTIONS || '1', 10);
const injectionCooldownMs = parseInt(process.env.CALMHIVE_INJECTION_COOLDOWN || '300000', 10); // 5 minutes default
...
function isConversationAlreadyInjected(bodyData) {
  ...
  // Check if we've injected into this conversation recently
  if (conversationSessions.has(sessionId)) {
    const sessionData = conversationSessions.get(sessionId);
    
    // Check injection count limit
    if (sessionData.injectionCount >= maxInjectionsPerSession) {
      if (debugMode) {
        console.error(`[Calmhive Debug] Session injection limit reached (${sessionData.injectionCount}/${maxInjectionsPerSession}): ${sessionId.slice(0, 20)}...`);
      }
      return true;
    }
    
    // Check cooldown period
    if (now - sessionData.lastSeen < injectionCooldownMs) {
      if (debugMode) {
        const cooldownRemaining = Math.round((injectionCooldownMs - (now - sessionData.lastSeen)) / 1000);
        console.error(`[Calmhive Debug] Session in cooldown period (${cooldownRemaining}s remaining): ${sessionId.slice(0, 20)}...`);
      }
      return true;
    }
  }
...
function markConversationInjected(bodyData) {
  ...
  if (sessionId) {
    ...
    const existing = conversationSessions.get(sessionId) || { injectionCount: 0 };
    conversationSessions.set(sessionId, {
      timestamp: existing.timestamp || Date.now(),
      lastSeen: Date.now(),
      messageCount: bodyData.messages.length,
      injectionCount: existing.injectionCount + 1
    });
  } else if (debugMode) {
...
function shouldInjectForRequest(bodyData) {
  // Check injection mode environment variable
  if (injectionMode === 'never') {
    if (debugMode) {
      console.error('[Calmhive Debug] Injection disabled by CALMHIVE_INJECTION_MODE=never');
    }
    return false;
  }
  ...
  // Enhanced detection for automated vs interactive requests
  const requestAnalysis = analyzeRequestType(bodyData);
  
  // Apply injection mode filtering
  if (injectionMode === 'interactive-only') {
    // Only inject for fresh user messages, not tool contexts
    if (requestAnalysis.requestType !== 'fresh-user-message') {
      if (debugMode) {
        console.error(`[Calmhive Debug] Skipping injection - interactive-only mode, request type: ${requestAnalysis.requestType}`);
      }
      return false;
    }
  }
  
  // Skip injection for tool execution contexts (in smart mode)
  if (injectionMode === 'smart' && requestAnalysis.requestType === 'tool-execution-context') {
    if (debugMode) {
      console.error('[Calmhive Debug] Skipping injection - automated tool execution context');
    }
    return false;
  }

  // Skip injection if this appears to be an automated continuation (in smart mode)
  // (many messages, recent tool use, no fresh human input)
  if (injectionMode === 'smart' && requestAnalysis.hasRecentToolUse && requestAnalysis.messageCount > 5) {
    if (debugMode) {
      console.error('[Calmhive Debug] Skipping injection - automated continuation with recent tool use');
    }
    return false;
  }
...
function modifyRequestBody(body) {
  ...
  try {
    ...
    if (bodyData.messages && Array.isArray(bodyData.messages)) {
      ...
      if (modified) {
        // Only log when actual injection occurs
        console.error(`[Calmhive Interceptor] Rules injected (session #${totalInjections})`);
    ...
    if (isClaudeAPI(url)) {
      ...
      if (modifiedOptions.body) {
        ...
        if (modifiedBody !== modifiedOptions.body) {
          ...
          // Logging moved inside modifyRequestBody to only log actual injections
        }
      }
    ...
    req.end = function(chunk, encoding, callback) {
      ...
      if (requestBody && claudeMdContent && ruleInjectionEnabled) {
        ...
        if (modifiedBody !== requestBody) {
          ...
          // Logging is handled inside modifyRequestBody to ensure consistency
