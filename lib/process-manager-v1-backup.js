 ...
 * EXPERIMENTAL: lib/process-manager-v2.js is a modular refactor being tested
 *
...
class ProcessManager {
  ...
  setupCleanup() {
    ...
    const cleanup = async () => {
      console.log('🧹 ProcessManager cleanup initiated...');

      for (const [sessionId, processInfo] of this.activeProcesses.entries()) {
        try {
          console.log(`🛑 Terminating process for session ${sessionId} (PID: ${processInfo.pid})`);

          // Use process group killing to ensure all child processes are terminated
          if (processInfo.process && !processInfo.process.killed) {
            // Try graceful shutdown first
            processInfo.process.kill('SIGTERM');

            // Give process 5 seconds to exit gracefully, then force kill
            setTimeout(() => {
              if (!processInfo.process.killed) {
                console.log(`⚡ Force killing process ${processInfo.pid}`);
          ...
          if (processInfo.caffeinatePid) {
            try {
              ...
              console.log(`🛑 Terminated caffeinate process ${processInfo.caffeinatePid}`);
            } catch (e) {
              // Caffeinate might already be dead
            }
          }

          // Update database to reflect process termination
          if (this.db) {
            try {
              ...
              console.error(`Failed to update session ${sessionId} status:`, e.message);
            }
          }

        } catch (e) {
          console.error(`Error cleaning up process ${processInfo.pid}:`, e.message);
        }
      }
    ...
    // Also handle unhandled rejections that might leave processes orphaned
    process.once('unhandledRejection', (reason, promise) => {
      console.error('🚨 Unhandled rejection detected, cleaning up processes:', reason);
      cleanup().finally(() => {
        process.exit(1);
      });
    });
  ...
  /**
   * Cleans up a specific process and updates its session status
   *
   * @param {string} sessionId - The session identifier
   * @param {string} status - Final status ('error', 'stopped', 'completed')
   * @param {string} errorMessage - Optional error message
   */
  async _cleanupProcess(sessionId, status = 'stopped', errorMessage = null) {
    try {
      // Remove from active tracking
      const processInfo = this.activeProcesses.get(sessionId);
      if (processInfo) {
        console.log(`🧹 Cleaning up process for session ${sessionId} (PID: ${processInfo.pid})`);
      ...
      console.error(`Failed to cleanup process for session ${sessionId}:`, error.message);
    }
  }
  ...
  async runSingleIteration(session, iterationNum, logStream, adaptiveRetry, progressTracker = null) {
    return new Promise((resolve) => {
      ...
      claudeProcess.on('error', (error) => {
        console.error(`🚨 Process error for session ${session.id}:`, error.message);
        this._cleanupProcess(session.id, 'error', error.message);
      });

      // Ensure cleanup happens on unexpected exit
      claudeProcess.on('spawn', () => {
        console.log(`🚀 Process spawned for session ${session.id} (PID: ${claudeProcess.pid})`);
      });
...
lets bee friends`;
      ...
      claudeProcess.on('exit', (code) => {
        ...
        // Ensure caffeinate is also cleaned up if iteration completes
        if (processInfo && processInfo.caffeinatePid) {
          try {
            process.kill(processInfo.caffeinatePid, 'SIGTERM');
          } catch (e) {
            // Caffeinate might already be dead
