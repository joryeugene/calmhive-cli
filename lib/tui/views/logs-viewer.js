...
class LogsView {
  ...
  createComponents() {
    ...
    this.logBox = blessed.log({
      ...
      tags: true,
      parseTags: false  // Safer for raw log content
    });
  }
  ...
  formatLogLine(line) {
    ...
    // Escape blessed tags properly to prevent parsing crashes
    ...
    // Simple text-based coloring without blessed tags for safety
    // We'll use ANSI escape codes instead of blessed tags
  ...
  escapeBlessedTags(text) {
    ...
    // Replace all curly braces with safe equivalents
    return text
      .replace(/\{/g, '\\{')
      .replace(/\}/g, '\\}');
  }
