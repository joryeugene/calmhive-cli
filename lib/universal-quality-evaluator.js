...
const MockClaudeEvaluator = require('./mock-claude-evaluator');
...
class UniversalQualityEvaluator extends AFkEvaluator {
  constructor() {
    ...
    // Initialize mock evaluator for fast testing
    this.mockEvaluator = new MockClaudeEvaluator();
    this.isMockingEnabled = MockClaudeEvaluator.isMockingEnabled();
    
    if (this.isMockingEnabled) {
      console.log('🎭 Mock Claude evaluation enabled for fast testing');
    }
...
`
  ...
  async evaluateCliCommand(command, output, errorOutput = '', expectedBehavior = '') {
    ...
    return this.evaluateWithConfidence('cliCommand', '', options);
  }
